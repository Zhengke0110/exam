...
import { AnswerType, GetAnswerInfoApi } from "@/services";
...
onMounted(async () => {
  ...
  if (id && type) {
    ...
    const answerData = Array.isArray(data) ? data[0] : data;
    if (answerData) {
      Object.assign(formData, answerData);
      console.log("获取到的数据：", answerData);
...
:deep(.van-button--default):not(.van-button--plain) {
  color: #666;
  background: #f5f5f5;
  border-color: #ddd;
}
